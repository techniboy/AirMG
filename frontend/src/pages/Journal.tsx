...
export default function Journal() {
  ...
  const grouped = questions.reduce<Record<string, JournalQuestion[]>>((acc, q) => {
    ...
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      {Object.entries(grouped).map(([category, qs]) => (
        <div key={category} className="space-y-2">
          <div className="flex items-center gap-2 px-1">
            ...
            <span className="text-xs uppercase tracking-widest text-text-tertiary">{category}</span>
          </div>
                  ...
                  >
                    ...
                    <div className="ml-4 shrink-0">
                      {isSaving ? (
                        ...
                        >
                          {checked && (
                            <svg viewBox="0 0 10 8" className="h-3 w-3 fill-surface-base">
      ...
      {questions.length > 0 && (
        <p className="text-xs text-text-tertiary text-center">
          Toggle behaviours to log whether they happened today. After a few days, Insights will rank their effect on your metrics.
        </p>
      )}
    </div>
  );
