  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <div className="flex items-start justify-between">
        ...
        {recs.length > 0 && (
          <span className="text-sm text-text-tertiary">{recs.length} recommendations</span>
        )}
      </div>
      ...
      <div className="space-y-3">
        {sorted.map((rec, i) => (
          <Card
            key={i}
            className="border-hairline bg-surface-raised p-5"
          >
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0 flex-1 space-y-2">
                ...
                <p className="text-sm text-text-primary leading-relaxed">{rec.message}</p>
              </div>
            </div>
          </Card>
        ))}
      ...
      <p className="text-xs text-text-tertiary">
        These are deterministic rule-based recommendations, not AI-generated advice.
