  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <p className="text-sm text-text-secondary">
        Metric correlations across your history. Positive r means the two metrics tend to move together;
      ...
      {!loading && correlations.length === 0 && (
        <Card className="border-hairline bg-surface-raised p-8 text-center text-text-tertiary">
          Not enough overlapping history to compute correlations yet.
          Sync more data and check back.
        </Card>
      )}
      ...
      <div className="space-y-3">
        {sorted.map((c, i) => {
          ...
          return (
            <Card key={i} className="border-hairline bg-surface-raised p-5 space-y-3">
              {/* Header */}
              <div className="flex items-start justify-between gap-3">
                <div className="font-semibold text-text-primary leading-snug">{title}</div>
