...
def analyze(
    ...
    for question_key, day_answers in journal.items():
        ...
        if len(with_vals) < MIN_GROUP_SIZE:
            continue
        if len(without_vals) < MIN_GROUP_SIZE:
            continue

        d = _cohens_d(with_vals, without_vals)
        ...
        diff_pct = abs(with_mean - without_mean) / without_mean * 100 if without_mean != 0 else 0.0
        higher_lower = "higher" if with_mean > without_mean else "lower"
        direction_word = (
            "better"
            if direction == "positive"
            else ("worse" if direction == "negative" else "similar")
        )
        sentence = (
