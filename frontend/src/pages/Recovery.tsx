...
export default function Recovery() {
  ...
  const { data, loading, error } = useApi<RecoveryDetail>(`/api/recovery/${selectedDay}`);
        ...
        <>
          ...
          <Card className="border-hairline bg-surface-raised p-8">
            <div className="flex flex-col items-center gap-4">
              ...
              <div className="text-sm text-text-tertiary">Recovery score for {selectedDay}</div>
            </div>
          </Card>

          {/* Contributing factors */}
          <div>
            <div className="mb-3 text-xs uppercase tracking-widest text-text-tertiary">Contributing Factors</div>
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              ...
              <MetricCard
                label="Sleep Perf"
                value={data.sleep_performance != null ? `${Math.round(data.sleep_performance)}` : '--'}
                unit="%"
                color="text-accent"
              />
            </div>
          ...
          {trendPoints.length >= 2 && (
            <Card className="border-hairline bg-surface-raised p-4 space-y-2">
              <div className="text-sm font-medium text-text-secondary">Recovery Trend</div>
