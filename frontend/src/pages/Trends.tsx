...
type MetricKey = 'recovery' | 'strain' | 'hrv_rmssd' | 'resting_hr' | 'sleep_performance' | 'sleep_minutes' | 'resp_rate';
type RangeKey = '7d' | '30d' | '90d';

const METRICS: Array<{ key: MetricKey; label: string; color: string; unit: string; domain?: [number, number] }> = [
  { key: 'recovery', label: 'Recovery', color: '#18C98B', unit: '%', domain: [0, 100] },
  { key: 'strain', label: 'Strain', color: '#E8743B', unit: '', domain: [0, 21] },
  { key: 'hrv_rmssd', label: 'HRV', color: '#A879FF', unit: 'ms' },
  { key: 'resting_hr', label: 'Resting HR', color: '#FF4F73', unit: 'bpm' },
  { key: 'sleep_performance', label: 'Sleep Perf', color: '#5BE0C7', unit: '%', domain: [0, 100] },
  { key: 'sleep_minutes', label: 'Sleep Duration', color: '#5C6FB1', unit: 'min' },
...
export default function Trends() {
  ...
  const avg = validPoints.length > 0
    ...
    : null;
  const min = validPoints.length > 0 ? Math.min(...validPoints.map((p) => p.value!)) : null;
  const max = validPoints.length > 0 ? Math.max(...validPoints.map((p) => p.value!)) : null;
    ...
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <Card className="border-hairline bg-surface-raised p-4 space-y-3">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-base font-semibold text-text-primary">{metricMeta.label}</div>
            <div className="text-xs text-text-tertiary">{validPoints.length} readings · {selectedRange}</div>
          </div>
          {avg !== null && (
            <div className="text-right">
              ...
              <div className="text-xs text-text-tertiary">avg{metricMeta.unit ? ` ${metricMeta.unit}` : ''}</div>
            </div>
          )}
        </div>
        ...
        {validPoints.length > 0 && (
          <div className="flex gap-6 border-t border-hairline pt-3 text-sm text-text-tertiary">
            <div>
              Avg <span className="text-text-primary font-medium">{avg?.toFixed(1)}</span>
            </div>
            <div>
              Min <span className="text-text-primary font-medium">{min?.toFixed(1)}</span>
            </div>
            <div>
              Max <span className="text-text-primary font-medium">{max?.toFixed(1)}</span>
            </div>
            <div>
              Days <span className="text-text-primary font-medium">{validPoints.length}</span>
            </div>
          </div>
        )}
      </Card>
