...
def evaluate(conn: sqlite3.Connection, day: str | None = None) -> ReadinessResult:
    ...
    # Build signals
    hrv_sig = _hrv_signal(rows, hrv_baseline)
    rhr_sig = _rhr_signal(rows, rhr_baseline)
    load_sig, acwr = _load_signal(rows)
    sleep_sig = _sleep_debt_signal(rows, sleep_need_minutes)
