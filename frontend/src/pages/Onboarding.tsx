			...
			const poll = setInterval(async () => {
				...
				try {
					...
					if (attempts > 20) {
						...
						setSyncing(false);
						setError(
							"Sync timed out. You can proceed and retry from Settings.",
						);
					}
				}
			}, 2000);
					...
					<>
						<div className="space-y-2">
							...
							<p className="text-sm text-text-secondary">
								...
								Sign in with Google to grant access — your data stays on your
								device.
							</p>
						</div>
						...
						<p className="text-xs text-center text-text-tertiary">
							Read-only access. You can revoke this at any time from your Google
							account.
						</p>
					</>
				)}
					...
					<>
						<div className="space-y-2">
							...
							{authStatus?.email && (
								<p className="text-sm text-text-secondary">
									Connected as{" "}
									<span className="text-accent">{authStatus.email}</span>
								</p>
							)}
							<p className="text-sm text-text-secondary">
								Pull your sleep, recovery, workouts and vitals from the last 90
								days.
							</p>
						</div>
						...
						{syncStatus && (
							...
							>
								{syncStatus.message ??
									(syncStatus.status === "running" ? "Syncing…" : "Done!")}
							</div>
						)}
				...
				{step === "done" && (
					<>
						<div className="space-y-2 text-center">
							...
							<p className="text-sm text-text-secondary">
								Your health data has been synced. Head to the dashboard to
								explore your recovery, sleep and strain trends.
							</p>
						</div>
						<Button
							className="w-full bg-accent text-surface-base hover:bg-accent-hover font-semibold"
