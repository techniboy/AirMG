  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <div className="flex items-center justify-between">
        ...
        {workouts.length > 0 && (
          <span className="text-sm text-text-tertiary">{workouts.length} sessions</span>
        )}
      </div>
            ...
            <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex flex-wrap gap-x-5 gap-y-1 text-sm">
                  {w.calories != null && (
                    <div>
                      ...
                      <span className="font-medium text-metric-amber">{Math.round(w.calories).toLocaleString()}</span>
                    </div>
                  )}
                  {w.avg_hr != null && (
                    <div>
                      <span className="text-text-tertiary">Avg HR </span>
                      <span className="font-medium text-metric-rose">{Math.round(w.avg_hr)}</span>
                  ...
                  {w.max_hr != null && (
                    <div>
                      ...
                      <span className="font-medium text-status-critical">{Math.round(w.max_hr)}</span>
                      <span className="text-text-tertiary"> bpm</span>
                    </div>
                  )}
                </div>
              ...
              {w.strain != null && (
                <div className="flex flex-col items-end gap-1">
                  <span className={`text-2xl font-bold tabular-nums ${strainColor(w.strain)}`}>
                    {w.strain.toFixed(1)}
                  </span>
                  <Badge
                    variant="outline"
