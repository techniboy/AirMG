...
def upsert_sleep_session(conn: sqlite3.Connection, start_ts: int, end_ts: int,
    efficiency: float | None = None, stages_json: str | None = None,
    resting_hr: int | None = None, avg_hrv: float | None = None,
    source: str = "google-health") -> None:
    conn.execute(
...
def upsert_workout(conn: sqlite3.Connection, start_ts: int, end_ts: int,
    workout_type: str | None = None, calories: float | None = None,
    avg_hr: int | None = None, max_hr: int | None = None,
    strain: float | None = None, source: str = "google-health") -> None:
    conn.execute(
...
def upsert_daily_metrics(conn: sqlite3.Connection, data: dict[str, Any]) -> None:
    cols = ["day", "recovery", "strain", "sleep_performance", "hrv_rmssd", "resting_hr",
    ...
    conn.execute(f"INSERT INTO daily_metrics ({', '.join(keys)}) VALUES ({placeholders}) ON CONFLICT(day) DO UPDATE SET {updates}", present)
    conn.commit()

def upsert_baseline(conn: sqlite3.Connection, metric: str, mean: float, spread: float,
    n_valid: int, nights_since_update: int = 0, status: str = "calibrating") -> None:
    conn.execute(
        """INSERT INTO baselines (metric, mean, spread, n_valid, nights_since_update, status, updated_at)
           VALUES (?, ?, ?, ?, ?, ?, ?) ON CONFLICT(metric) DO UPDATE SET
             ...
             nights_since_update=excluded.nights_since_update, status=excluded.status, updated_at=excluded.updated_at""",
        (metric, mean, spread, n_valid, nights_since_update, status, int(time.time())))
    conn.commit()

def upsert_journal_entry(conn: sqlite3.Connection, day: str, question_key: str, answer: str, created_at: int | None = None) -> None:
    conn.execute(
...
def upsert_steps(conn: sqlite3.Connection, day: str, total: int, source: str = "google-health") -> None:
    conn.execute("INSERT INTO steps (day, total, source) VALUES (?, ?, ?) ON CONFLICT(day) DO UPDATE SET total=excluded.total", (day, total, source))
    conn.commit()

def set_sync_state(conn: sqlite3.Connection, data_type: str, last_synced_ts: int, last_token: str | None = None) -> None:
    conn.execute(
        """INSERT INTO sync_state (data_type, last_synced_ts, last_token) VALUES (?, ?, ?)
           ON CONFLICT(data_type) DO UPDATE SET last_synced_ts=excluded.last_synced_ts, last_token=excluded.last_token""",
        (data_type, last_synced_ts, last_token))
    conn.commit()

def set_profile(conn: sqlite3.Connection, key: str, value: str) -> None:
    conn.execute("INSERT INTO profile (key, value) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value=excluded.value", (key, value))
    conn.commit()
